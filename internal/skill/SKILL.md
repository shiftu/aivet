...
- 一件工具往往不止一个模型槽。id 以 `.models` 结尾的项（标题「其他模型」）说的是
  **主模型之外、配置里还写着的那些**：codex 的 `review_model`、Claude Code 的三个别名、
  hermes/pi/dsh 各自的模型菜单。报 `warn` 表示那几个网关没有 —— 现在不影响主模型，
  但用户在工具里切过去就会 404，所以值得顺手改掉；报 `skip` 表示没核对成（离线 / 拿不到 key /
  网关没有清单接口），那就别拿它当「没问题」。
- cc-switch 那一节（`ccswitch.current.<工具>`）说的是**谁在管**，不是「同步没同步」。
  aivet 的立场：原生配置优先，官方登录为正，cc-switch 是 fallback。所以：
  - 「原生官方登录 · cc-switch 没接管」「另存 N 个备用未启用」都是正常状态，**不要**建议用户迁到 cc-switch；
  - 只有原生探不通时它才会推荐 `cc-switch use …`，而且推荐的那个已经探过是通的。照它给的命令执行即可；
  - 「记录过时了」= 原生现在是官方登录、cc-switch 还记着旧网关：现在没事，但别在 cc-switch 里随手再切一次。
